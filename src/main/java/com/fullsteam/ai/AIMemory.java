...
public class AIMemory {
    ...
    public void observeLocation(StrategicLocation location) {
        int locationId = location.getId();
        int currentController = location.getControllingPlayerId();
        
        // Check if control has changed
        Integer previousController = locationControlHistory.get(locationId);
        if (previousController == null || !previousController.equals(currentController)) {
            locationControlHistory.put(locationId, currentController);
            locationLastChanged.put(locationId, System.currentTimeMillis());
        }
    }
