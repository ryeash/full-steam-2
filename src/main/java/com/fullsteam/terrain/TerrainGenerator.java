...
//        generateEnvironmentalFeatures();
    ...
    private void generateObstacles() {
        ...
        terrainMetadata.put("targetObstacleCount", targetObstacleCount);

        // Generate obstacles with collision checking
        int attemptsPerObstacle = 50; // Maximum attempts to find a clear position
        ...
        // Store actual count
        terrainMetadata.put("obstacleCount", successfulPlacements);
    }
    ...
    private boolean isObstaclePositionClear(Obstacle obstacle) {
        ...
        for (TerrainFeature feature : terrainFeatures) {
            ...
            if (distance < feature.getSize() + totalRadius) {
                return false;
            }
        }
        ...
        // Check against compound obstacles
        for (CompoundObstacle compoundObstacle : compoundObstacles) {
            if (compoundObstacle.overlapsWithCircle(position, totalRadius)) {
                return false;
            }
        }
    ...
    private boolean isCompoundObstaclePositionClear(CompoundObstacle compoundObstacle) {
        ...
        for (Obstacle obstacle : generatedObstacles) {
            ...
            double obstacleRadius = obstacle.getBoundingRadius();
