...
public class ShapedObstacle extends Obstacle {
    ...
    public static ShapedObstacle createRandomObstacle(double x, double y) {
        ...
        // Add slight position chaos for more natural placement
        double xOffset = random.nextGaussian() * 5;
        double yOffset = random.nextGaussian() * 5;

        return new ShapedObstacle(Config.nextId(), x + xOffset, y + yOffset, randomType);
    }

    /**
     * Factory method to create extra chaotic obstacles with maximum randomization.
     */
    public static ShapedObstacle createChaoticObstacle(double x, double y) {
        ThreadLocalRandom random = ThreadLocalRandom.current();
